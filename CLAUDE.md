...
- **`publish-version`** — publish the latest tested APK as a GitHub release of the fork: tag
  `<version>` — the **bare** version, **no `v` prefix** (白い熊, 2026-09-05), matching the APK filename
  — attach the APK, refresh the README badge + `CHANGELOG.md`, keep the default branch on `custom`.
  Pin `gh` with `-R ShiroiKuma0/shiroikuma-kxkb` (the `upstream` remote otherwise wins).
